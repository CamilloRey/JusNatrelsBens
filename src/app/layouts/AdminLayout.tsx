    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", display: 'flex', minHeight: '100vh', background: '#f5f0ea' }}>
      ...
      <aside style={{ width: sidebarOpen ? 240 : 0, background: C.dark, overflow: 'hidden', transition: 'width 0.3s', flexShrink: 0 }}>
        <div style={{ padding: '24px 20px', minWidth: 240 }}>
          ...
          {MENU.map(m => (
            <NavLink key={m.route} to={m.route}
              style={({ isActive }) => ({
                ...
                marginBottom: 4, textAlign: 'left', textDecoration: 'none', position: 'relative',
              })}>
              {({ isActive }) => (
                <>
                  <Icon type={m.icon as any} size={18} color={isActive ? C.red : '#6b5e52'} />
                  {m.label}
                  {m.route === ROUTES.admin.messages && unreadCount > 0 && (
                    <span style={{ marginLeft: 'auto', minWidth: 20, height: 20, borderRadius: 10, background: '#dc2626', color: '#fff', fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 6px' }}>{unreadCount}</span>
                  )}
                </>
              )}
            </NavLink>
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', marginTop: 24, paddingTop: 20 }}>
            <button onClick={handleLogout} style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', padding: '11px 14px', border: 'none', borderRadius: 10, background: 'transparent', color: '#6b5e52', cursor: 'pointer', fontSize: 14, textAlign: 'left' }}>
      ...
      <main style={{ flex: 1, overflow: 'auto' }}>
        <header style={{ padding: '16px 28px', background: '#fff', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, fontWeight: 700, color: C.dark, margin: 0 }}>Administration</h1>
          </div>
