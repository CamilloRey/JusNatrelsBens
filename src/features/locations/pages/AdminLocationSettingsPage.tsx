...
export default function AdminLocationSettingsPage() {
  const { locationSettings, updateLocationSettings, logActivity } = useData();
  ...
  const save = () => {
    ...
    logActivity('Paramètres points de vente modifiés', 'Types mis à jour', 'location');
  };
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h2 style={{ ...CSS.heading, fontSize: 22, margin: 0 }}>Paramètres Points de vente</h2>
          <p style={{ fontSize: 12, color: C.muted, margin: '6px 0 0' }}>
            Gérez les types et les régions utilisées pour filtrer.
          </p>
        </div>
        <button
          type="button"
      ...
      <div style={{ background: '#fff', borderRadius: 14, padding: 18, border: `1px solid ${C.border}`, marginBottom: 16 }}>
        <p style={{ fontSize: 12, fontWeight: 700, color: C.dark, margin: '0 0 10px' }}>Types de points de vente</p>
      ...
      <div style={{ background: '#fff', borderRadius: 14, padding: 18, border: `1px solid ${C.border}` }}>
        <p style={{ fontSize: 12, fontWeight: 700, color: C.dark, margin: '0 0 10px' }}>Régions (fixes)</p>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {form.regions.map(region => (
            <span key={region} style={{
              ...
              {region === 'Montreal' ? 'Montréal' : region === 'Rive Sud' ? 'Rive Sud de Montréal' : 'Rive Nord de Montréal'}
            </span>
          ))}
        </div>
      </div>
